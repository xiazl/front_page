...
@Form.create()
class CreateByQtyModal extends PureComponent {
  state = {
    outCardType: ['1'],
    projectName: '',
    gNames: [],
  ...
  okHandle = () => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      createTask({ ...fieldsValue, outCardType: 1 });
    });
  };
  ...
  autoTransformWhenProjectOnBlur = e => {
    ...
    if (index >= 0) {
      const cardCount = value.substr(index + 1).replace(/[^0-9.]/gi, '');
      if (cardCount) {
        fieldsValue.cardCount = cardCount;
      }
    }
  ...
  render() {
    ...
    const { outCardType, projectName, gNames, bankNames: bNames } = this.state;
    ...
    return (
      ...
      >
        ...
        <FormItem {...formItemProps} label="卡片数量">
          {form.getFieldDecorator('cardCount', {
            rules: [
              { required: true, whitespace: true, message: '请输入卡片数量！' },
              {
                pattern: /^(\d+)$/,
                message: '请输入合法整数',
              },
            ],
          })(<Input placeholder="请输入卡片数量" addonAfter="张" />)}
        </FormItem>
        <FormItem {...formItemProps} label="单张卡金额">
          ...
          })(<Input placeholder="请输入单张卡金额" addonAfter="万" disabled />)}
        </FormItem>
...
export default CreateByQtyModal;
